...
class PhysicsObject(object):
    def __init__(self, position=None, quaternion=None, linear_velocity=None, angular_velocity=None):
        ...
        # self.position: np.ndarray = position if position is not None else np.zeros(3)  # original
        ...
        # self.quaternion: np.ndarray = quaternion if quaternion is not None else np.ones(4)  # original
        # self.quaternion: Union[ndarray, List] = quaternion if quaternion is not None else [1, 1, 1, 1]

        # self.linear_velocity: np.ndarray = linear_velocity if linear_velocity is not None else np.zeros(3)  # original
        # self.linear_velocity: Union[ndarray, List] = linear_velocity if linear_velocity is not None else [1, 1, 1]
        # self.angular_velocity: np.ndarray = angular_velocity if angular_velocity is not None else np.zeros(3)  # original
        # self.angular_velocity: Union[ndarray, List] = angular_velocity if angular_velocity is not None else [0, 0, 0]
        # self._euler_angles: Optional[np.ndarray] = np.zeros(3)  # original
        # self._euler_angles: Optional[List] = [0, 0, 0]
        # self._rotation_mtx: Optional[ndarray] = zeros((3, 3))  # original
        # self._has_computed_rot_mtx = False
        # self._has_computed_euler_angles = False

    def decode_car_data(self, car_data: ndarray):
        """
        ...
        """
        self.position, self.quaternion, self.linear_velocity, self.angular_velocity = car_data[:3], \
                                                                                      car_data[3:7], \
                                                                                      car_data[7:10], \
                                                                                      car_data[10:],
    ...
    def decode_ball_data(self, ball_data: ndarray):
        """
        ...
        """
        self.position, self.linear_velocity, self.angular_velocity = ball_data[:3], ball_data[3:6], ball_data[6:9]
...
class PhysicsObjectDecode(object):
    def __init__(self, position=None, quaternion=None, linear_velocity=None, angular_velocity=None):
        self.position, self.quaternion, self.linear_velocity, self.angular_velocity, self._euler_angles, \
        self._rotation_mtx, self._has_computed_rot_mtx, self._has_computed_euler_angles = \
            position if position is not None else None, \
            quaternion if quaternion is not None else None, \
            linear_velocity if linear_velocity is not None else None, \
            angular_velocity if angular_velocity is not None else None, None, \
            None, False, False
        # self.position: np.ndarray = position if position is not None else np.zeros(3)  # original
        # self.position: Union[ndarray, List] = position if position is not None else [0, 0, 0]
